...
# Load model
model = Generator(in_channels=3, out_channels=3, num_init_features=64, scale_factor=args.scale_factor).to(device)
...
for i, data in enumerate(dataloader):
    ...
    src_img = cv2.imread(f"{args.outf}/{filename}_esrgan.png")
    ...
    total_niqe_value += cal_niqe(f"{args.outf}/{filename}_esrgan.png")
