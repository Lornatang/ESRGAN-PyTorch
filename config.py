...
if mode == "train":
    # Configure dataset.
    train_dir             = "data/DFO2K/train"                    # The address of the training data set.
    valid_dir             = "data/DFO2K/valid"                    # Verify the address of the data set.
    image_size            = 128                                   # High-resolution image size in the training data set.
    ...
    p_scheduler           = CosineAnnealingWarmRestarts(p_optimizer, p_epochs // 4, 1, 1e-7)  # Generate model scheduler during generator training.
    d_scheduler           = MultiStepLR(d_optimizer, list(map(int, milestones)), 0.5)         # Identify the model scheduler during adversarial training.
    g_scheduler           = MultiStepLR(g_optimizer, list(map(int, milestones)), 0.5)         # Generate model scheduler during adversarial training.
